...
#wsi/lab3/two_player_games/game.py
class Pick(Game):
...
class PickState(State):
    ...
    def get_children(self):
        ...
        for move in self.get_moves():
            ...
            children.append(self.make_move(move))
        return children

    def __str__(self) -> str:
        return f"n: {self.n}, aim_value: {self.aim_value}" \
               f"\nCurrent player: {self._current_player.char}, Numbers: " \
               f"{'[]' if not self.current_player_numbers else self.current_player_numbers}," \
               f"\nOther player: {self._other_player.char}, Numbers: " \
               f"{'[]' if not self.other_player_numbers else self.other_player_numbers}"
